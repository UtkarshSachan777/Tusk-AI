...
const LiveFraudDemo = () => {
  ...
  const analyzeTransaction = async () => {
    ...
    try {
      // Use the advanced ML fraud detection function
      const response = await supabase.functions.invoke('advanced-ml-fraud-detection', {
        body: {
          transaction: {
            ...transaction,
            ...
            user_age: transaction.customerAge,
            account_balance: transaction.accountBalance,
            previous_transactions: transaction.recentTransactions.map(amt => ({ amount: amt })),
      ...
      const mlResult = response.data.result;
      setResult({
        riskScore: mlResult.ensemble_score,
        prediction: mlResult.prediction as 'legitimate' | 'fraudulent' | 'suspicious',
        riskFactors: mlResult.risk_factors,
        confidence: mlResult.confidence,
        recommendedAction: mlResult.recommended_actions?.[0] || 'Review transaction'
      });
    } catch (error) {
